...
test_pipeline_dry_run_default_plan_excludes_manual_steps() {
  ...
  assert_contains "$contents" '- `11-ultra-review.md` (automated' \
    "Expected the new automated Step 11 (ultra-review) to appear in the default plan."
  assert_contains "$contents" '- `14-rebase.md` (automated' \
    "Expected automated steps to run through Step 14 after renumbering."
  ...
  assert_not_contains "$contents" '- `15-merge.md` (manual, timeout' \
    "Did not expect manual Step 15 in the planned-steps section by default."
  assert_not_contains "$contents" '- `16-cleanup.md` (manual, timeout' \
...
test_pipeline_dry_run_include_manual_adds_manual_checklist_steps() {
  ...
  local contents
  contents=$(cat "$manifest")
  assert_contains "$contents" '- `15-merge.md` (manual' \
    "Expected manual Step 15 in the planned list when --include-manual is set."
  assert_contains "$contents" '- `16-cleanup.md` (manual' \
...
test_pipeline_dry_run_only_flag_plans_single_step() {
  setup_pipeline_fixture
  run_pipeline_dry_run --only 11-ultra-review.md
...
test_pipeline_dry_run_start_from_includes_step_and_successors() {
  ...
  assert_contains "$contents" '- `14-rebase.md` (automated' \
    "Expected --start-from to include later automated steps through Step 14."
  assert_not_contains "$contents" '- `10-address-findings.md`' \
...
STEP_TIMEOUTS+=("11-ultra-review.md=9999")
EOF
  ...
  local contents
  contents=$(cat "$manifest")
  assert_contains "$contents" '- `11-ultra-review.md` (automated, timeout 9999s)' \
