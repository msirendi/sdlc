...
- Branch name (e.g. `<handle>/my-fix-branch`)
...
1. **Create the feature branch** from the current `main` HEAD:
   ...
   git checkout -b <handle>/my-fix-branch
   ```

2. **Publish the branch** to the remote so it tracks correctly:
   ```
   git push -u origin <handle>/my-fix-branch
   ```

3. **Create and open a worktree** for the branch so `main` stays clean:
   ```
   git worktree add ../worktrees/my-fix-branch <handle>/my-fix-branch
...
- Local branch `<handle>/my-fix-branch`
- Remote branch `origin/<handle>/my-fix-branch` tracking the local branch
- New worktree checked out to `<handle>/my-fix-branch`
...
- Remote branch `origin/<handle>/my-fix-branch` exists and tracks the local branch.
