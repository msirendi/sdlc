...
- Feature branch (e.g. `<handle>/my-fix-branch`)
...
1. **Check if a rebase is needed:**
   ...
   git log --oneline <handle>/my-fix-branch..origin/main
   ```
...
3. **Perform the rebase:**
   ```
   git checkout <handle>/my-fix-branch
...
6. **Force-push the rebased branch:**
   ```
   git push --force-with-lease origin <handle>/my-fix-branch
   ```
